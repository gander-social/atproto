...
describe('server', () => {
  ...
  it('preserves 404s.', async () => {
    const response = await fetch(`${network.gndr.url}/unknown`)
  ...
  // TODO(gndr) check on a different endpoint that accepts json, currently none.
  it.skip('limits size of json input.', async () => {
    const response = await fetch(
      `${network.gndr.url}/xrpc/com.atproto.repo.createRecord`,
      {
  ...
  it('compresses large json responses', async () => {
    const res = await request(
      `${network.gndr.url}/xrpc/app.gndr.feed.getTimeline`,
      {
        headers: {
          ...(await network.serviceHeaders(alice, 'app.gndr.feed.getTimeline')),
  ...
  it('does not compress small payloads', async () => {
    const res = await request(`${network.gndr.url}/xrpc/_health`, {
  ...
  it('healthcheck fails when dataplane is unavailable.', async () => {
    const { port } = network.gndr.dataplane.server.address() as AddressInfo
    await network.gndr.dataplane.destroy()
