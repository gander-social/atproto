...
// @NOTE temporary measure, see note on entrywaySession in gndr/src/auth-verifier.ts
describe('entryway auth', () => {
  ...
  beforeAll(async () => {
    ...
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_entryway_auth',
      gndr: {
  ...
  it('does not work on bad scopes', async () => {
    ...
    const token = await signer.sign(jwtPrivKey)
    const attempt = agent.app.gndr.actor.getProfile(
      { actor: sc.dids.bob },
      { headers: { authorization: `Bearer ${token}` } },
    )
